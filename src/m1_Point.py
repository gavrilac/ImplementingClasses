...
class Point(object):
    ...
    def __repr__(self):
        """Returns a string that represents the point object"""
    ...
    def move_to(self, x , y):
        point5 = Point(x,y)
        self.dist += self.get_distance_from(point5)
        self.x = x
        self.y = y
        self.count += 1

    def move_by(self, dx, dy):
        point5 = Point(self.x + dx, self.y + dy)
        self.dist += self.get_distance_from(point5)
        self.x = self.x + dx
        self.y = self.y + dy
        self.count += 1
    ...
    def get_distance_from_start(self):
        point8 = Point(self.startx, self.starty)
        return self.get_distance_from(point8)

    def get_distance_traveled(self):
        return self.dist

    def closer_to(self, point2, point3):
        dist1 = self.get_distance_from(point2)
        dist2 = self.get_distance_from(point3)
        if dist1 > dist2:
            return point3
        else:
            return point2
...
def run_test_clone():
   ...
   """
    # ------------------------------------------------------------------
    # : 5.DONE  Follow the same instructions as in TODO 3 above,
